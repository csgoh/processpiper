...
def test_sample11():
    with ProcessMap("Are we living in simulation?", colour_theme="TEALWATERS") as process_map:
        with process_map.add_pool("The World") as pool:
            with pool.add_lane("You") as you:
                ...
                g1 = you.add_element("Did boss walk past twice \nin the same manner?", GatewayType.EXCLUSIVE)
                t3 = you.add_element("Ask a stranger life purpose", ActivityType.TASK)
                g2 = you.add_element("Responded 'huh?' \nlike NPC?", GatewayType.EXCLUSIVE)
                start.connect(t1)
                t1.connect(t2)
                t2.connect(g1)
                final_simulated = you.add_element("The World is simulated", ActivityType.TASK)
                final_not_simulated = you.add_element("The World is not simulated", ActivityType.TASK)
...
# Connect all the elements
    ...
    text2diagram.render(diagram_str, "images/test/test_sample12.png", )
